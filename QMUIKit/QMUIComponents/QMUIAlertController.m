...
@property(nonatomic, strong) UIControl *qmuiMaskView;
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.qmuiMaskView];
...
- (void)viewDidLayoutSubviews {
    ...
    self.qmuiMaskView.frame = self.view.bounds;
...
- (UIControl *)qmuiMaskView {
    if (!_qmuiMaskView) {
        _qmuiMaskView = [[UIControl alloc] init];
        _qmuiMaskView.alpha = 0;
        _qmuiMaskView.backgroundColor = UIColorMask;
        [_qmuiMaskView addTarget:self action:@selector(handleMaskViewEvent:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _qmuiMaskView;
}
